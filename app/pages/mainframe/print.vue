...
const tab_items = ref([
  { id: 0, label: 'Print', icon: 'pi pi-chart-line' },
  { id: 1, label: 'Add Telemetry Template', icon: 'pi pi-list' },
...
rpc('scg.mainframe.get_printer_names', []).then(async (res) => {
  if (res?.error == null) {
    if (res?.data.length > 0) {
      ...
      selectedPrinter.value = { name: res?.data[0] }
    }
  }
  else {
    const msg = {
...
<template>
  <div class="content">
    <AppName appname="Print Telemetry" />
