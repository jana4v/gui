...
function sub_system_selection_changed() {
  const selected_sub_system_data = selected_sub_system_names.value.map(
    item => item.v,
  )
...
async function generate_ppt() {
  ...
  try {
    useAPIFetch(`/papert/generate_ppt`, { method: 'POST', body }).then(
      async (res) => {
        if (res?.error == null) {
          pptx.value.click()
        }
  ...
  <div class="content min-h-screen w-full">
    ...
    <div v-if="selected_test_plan == undefined" class="grid pt-4">
      <div class="col flex flex-column gap-2">
        <CheckBoxSelection
          v-model:selected-params="selectedParameters"
