...
source.value = ['systems#transmitter', 'systems#receiver', 'systems#transponder', 'subsystem1#table1', 'subsystem1#table2', 'subsystem2#table1']
console.log(source.value)
// const { data: sub_system_names } = await useAPIFetch(`gui/getSubSystemNames`)
subsystem.value = ['Transmitter', 'Receiver', 'Transponder']

function db_table_selected_from_menu(path: string) {
...
<template>
  ...
  <div class="grid">
    <div class="col-2 ml-0 pl-0">
      <tracs-database-tables-menu @table-selected="db_table_selected_from_menu" />
    </div>
    ...
    <div class="col-10 card mt-2" style="height: calc(100vh - 9rem);padding: 0.5rem;">
      ...
      <AgGridTable
        v-if="!is_systems_group" :key="componentKey" table_height="90vh" table_width="100%"
        app_name="tracs" :table_name="selected_db_table"
        :table_configuration_func="tracs_table_config_func"
      />
      <h3 v-if="group_name.length == 0">
        No table selected
      </h3>
