<template>
  <div class="tx-card card">
    ...
    <div v-if="form.name.trim() && form.code.trim() && form.modulation_type === 'PSK_PM'" class="mt-2">
      <TracsV2ModulationFormsPskPmForm
...
import { useTransmitterStore } from '@/stores/tracsV2/transmitter';
import { useTransmitterApi } from '@/composables/tracsV2/useTransmitterApi';
import type { Transmitter, PskPmDetails } from '@/composables/tracsV2/useTransmitterApi';
...
async function handleDelete() {
  ...
  const success = await store.remove(props.transmitter!.code);
