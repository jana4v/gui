...
const DEFAULT_TEST_PLANS = ['Detailed', 'Short', 'Quick'] as const;
  ...
  const base: ColDef[] = [
    {
      ...
      sort: 'asc',
      comparator: (a, b) => {
        const order = ['Detailed', 'Short', 'Quick'];
...
function buildRows(transmitters: Transmitter[]): TestPlanRow[] {
  ...
  for (const tx of transmitters) {
    ...
    for (const plan of DEFAULT_TEST_PLANS) {
...
async function load() {
  ...
  const list = (res.data.value as Transmitter[]) ?? [];
  ...
  rows.value = buildRows(transmitters);
}
