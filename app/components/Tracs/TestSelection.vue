...
<template>
  <div class="">
    <div class="text-2xl text-center text-primary-600 font-bold">
      Test Selection
    </div>
    <hr>
    <div class="grid md:grid-cols-3 gap-2 sm:grid-cols-1 mx-2">
      <div class="">
        ...
        <Select
          v-model="selected_test_phase" :options="test_phase_names" :filter="false" input-id="select_test_phase"
          option-label="value" option-value="value" placeholder=" Select Test Phase" class="w-full"
        />
      </div>
      <div class="">
        <p class="mb-0 text-xl">
        ...
        <Select
          v-model="selected_sub_test_phase" :options="sub_test_phase_names" filter option-label="value"
          option-value="value" input-id="Select_sub_test_phase" placeholder="Select Subsystem" class="w-full"
        />
      </div>
      ...
      <div class="">
        ...
        <Select
          v-model="selected_test_plan" :options="test_plan_names" filter option-label="value" option-value="value"
          input-id="Select_test_plan" placeholder="Select Test Plan" class="w-full"
        />
      </div>
    </div>
    <div class="grid pt-4">
      <div class="col-12">
        <div class="flex align-items-center ">
          <div
            v-for="(item, index) in tab_menu_items" :key="index" class="flex align-items-center justify-content-center p-2" :class="[{ 'font-bold text-primary-600 border-bottom-2 border-primary-600': active_tab_number === index }]"
            @click="active_tab_number = index"
          >
            <i class="p-mr-2" :class="[item.icon]" />
            <span class="ml-2 text-2xl text-secondary-200"> {{ item.label }} </span>
            <Checkbox
              v-model="checkboxStates[index]" class="ml-3 mr-2 sys_type_selection"
              :binary="true" @click.stop="check_box_selection_changed(index)"
            />
          </div>
        </div>
        ...
        <div v-if="active_tab_number === 0">
          <AgGridTable
            ref="tx_table_ref" :key="componentKey" table_height="80vh" table_width="100%" app_name="tracs"
            :use_local_data="true" :table_name="tx_plan_tbl" :get_url="tx_plan_url"
            :table_configuration_func="tracs_table_config_func"
        ...
        <div v-if="active_tab_number === 1">
          <AgGridTable
            ref="rx_table_ref" :key="componentKey" table_height="80vh" :use_local_data="true" table_width="100%"
        ...
        <div v-if="active_tab_number === 2">
          <AgGridTable
            ref="tp_table_ref" :key="componentKey" table_height="80vh" :use_local_data="true" table_width="100%"
