...
function rest_api_save(tx_data: any) {
  useAPIFetch('tracs/saveTransmitterData', { method: 'post', body: tx_data }).then((res) => {
    if (res.error.value == null) {
      const toast_msg = { severity: 'success', summary: 'Status', detail: 'Database Updated', life: 3000 }
...
async function save() {
  ...
  if (is_form_not_found.value)
    return
  const tx_name_is_valid = await name_validator.isValid(transmitter_name.value)
  const tx_code_is_valid = await code_validator.isValid(transmitter_code.value)
  if (!tx_name_is_valid) {
  ...
  <div>
    <div class="grid mt-1">
      ...
      <div class="col-4">
        ...
        <InputText
          id="transmitter_name" v-model="transmitter_name" placeholder="Example: C Transmitter 1"
          type="text" :class="{ 'p-invalid': tx_name_err_msg }" aria-describedby="text-error"
          :disabled="!isEditable"
        />
        <div><span id="text-error" class="p-error">{{ tx_name_err_msg || '&nbsp;' }}</span></div>
      </div>
      <div class="col-4">
        <p class="mb-0 text-xl w-10">
          Code
        ...
        <InputText
          id="transmitter_code" v-model="transmitter_code" :disabled="!isEditable"
          placeholder="Example: CTX1" type="text" :class="{ 'p-invalid': tx_code_err_msg }"
          aria-describedby="text-error"
        />
        <div><span id="text-error" class="p-error">{{ tx_code_err_msg || '&nbsp;' }}</span></div>
      </div>
      <div class="col-4">
        <p class="mb-0 text-xl w-10">
          Modulation
