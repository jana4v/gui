...
// 3 seconds for success messages
...
const BANNER_TIMEOUT_SUCCESS = 3000; const tableData = ref(props.data)
...
function init_table(settings, data) {
  if (hotTableRef.value?.hotInstance) {
    ...
    if (data !== null)
      hotTableRef.value.hotInstance.loadData(data) // Update table data
...
<template>
  <div class="mt-5" :class="[isDarkMode ? 'ht-theme-main-dark' : 'ht-theme-main']" style="width: 600px">
    ...
    <HotTable
      :key="counter" ref="hotTableRef" :settings="hotTblSettings" :data="tableData"
